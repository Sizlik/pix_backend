$ErrorActionPreference = "Stop"
$repoRoot = Split-Path -Parent $PSScriptRoot
Set-Location $repoRoot

$ruffTargets = @(
    "config.py",
    "errors.py",
    "main.py",
    "alembic/env.py",
    "alembic/versions/c8f2a4e6d901_order_chat_delivery.py",
    "alembic/versions/b7e1d3a9f4c2_add_user_addresses.py",
    "alembic/versions/d4e5f6a7b8c9_remove_telegram.py",
    "alembic/versions/e3b7c9d1a204_allow_extension_chat_source.py",
    "alembic/versions/f4c8a2d6b901_order_chat_inbox_email.py",
    "db/postgres.py",
    "db/redis.py",
    "db/address_repository.py",
    "db/notification_repository.py",
    "db/order_chat_email_repository.py",
    "db/repository.py",
    "db/link_preview_repository.py",
    "db/models/addresses.py",
    "db/models/order_chat.py",
    "db/moysklad_order_chat_repository.py",
    "db/order_chat_repository.py",
    "db/schemas/chat.py",
    "db/schemas/addresses.py",
    "db/schemas/link_preview.py",
    "db/schemas/notifications.py",
    "db/schemas/orders.py",
    "dependecies/addresses.py",
    "dependecies/chat.py",
    "dependecies/link_preview.py",
    "dependecies/notifications.py",
    "dependecies/operator_inbox.py",
    "dependecies/orders.py",
    "dependecies/order_chat.py",
    "dependecies/order_chat_email.py",
    "manager/addresses.py",
    "manager/chat_files.py",
    "manager/chat_realtime.py",
    "manager/chat_storage.py",
    "manager/link_preview.py",
    "manager/moysklad.py",
    "manager/phone_numbers.py",
    "manager/notification_realtime.py",
    "manager/notifications.py",
    "manager/order_changes.py",
    "manager/order_creation.py",
    "manager/order_identity.py",
    "manager/order_idempotency.py",
    "manager/order_chat.py",
    "manager/order_chat_auth.py",
    "manager/operator_inbox_realtime.py",
    "manager/order_chat_email.py",
    "manager/order_chat_email_dispatcher.py",
    "manager/bitrix.py",
    "manager/privoz_order.py",
    "manager/production_config.py",
    "manager/users.py",
    "routes/bitrix.py",
    "routes/addresses.py",
    "routes/chat.py",
    "routes/link_preview.py",
    "routes/notifications.py",
    "routes/operator_chat.py",
    "routes/orders.py",
    "utils/celery_worker.py",
    "scripts/check_production_config.py",
    "moysklad_webhooks_creator.py",
    "tests"
)

& ".\.venv\Scripts\python.exe" -m ruff check @ruffTargets
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

$alembicHeads = @(& ".\.venv\Scripts\python.exe" -m alembic heads)
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
$headLines = @($alembicHeads | Where-Object { $_ -match "\(head\)" })
if ($headLines.Count -ne 1 -or $headLines[0] -notmatch "^f4c8a2d6b901\s") {
    Write-Error "Expected one Alembic head at f4c8a2d6b901."
    exit 1
}

& ".\.venv\Scripts\python.exe" -m pytest tests -q
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
