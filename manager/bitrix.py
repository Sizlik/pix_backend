...
class BitrixABC(ABC):
    def __init__(self, settings: Settings | None = None):
        webhook_url = require_secret(
            (settings or get_settings()).bitrix_link,
            "bitrix",
        )
        self._link = webhook_url.rstrip("/") + "/"
...
class BitrixManagerFull:
    def __init__(self, settings: Settings | None = None):
        ...
        bitrix_link = require_secret(settings.bitrix_link, "bitrix")
        self._link = bitrix_link.rstrip("/") + "/"
        self.deal_manager = BitrixManager(BitrixCrmDeal(settings))
        self.product_manager = BitrixManager(BitrixCrmProduct(settings))
        self.contact_manager = BitrixManager(BitrixCrmContact(settings))
    ...
    def insert_products(self, orders: OrderCreate, bitrix_deal_id):
        ...
        for i in orders.model_dump().get("order_items"):
            product = AddProductFields(NAME=f"{i['link']}", DESCRIPTION=f"{i['link']}\n{i['comment']}").model_dump()
...
class BitrixCrmContact(BitrixABC):
    ...
    def list(self, filter_: dict = None, order: dict = None, select: list = None) -> dict:
        return requests.post(
            self._link + "crm.contact.list.json", json={"filter": filter_, "order": order, "select": select}
        ).json()
...
class BitrixCrmDeal(BitrixABC):
    ...
    def list(self, filter_: dict = None, order: dict = None, select: list = None) -> dict:
        return requests.post(
            self._link + "crm.deal.list.json", json={"filter": filter_, "order": order, "select": select}
        ).json()
...
class BitrixCrmProduct(BitrixABC):
    ...
    def list(self, filter_: dict = None, order: dict = None, select: list = None) -> dict:
        return requests.post(
            self._link + "crm.product.list.json", json={"filter": filter_, "order": order, "select": select}
        ).json()
